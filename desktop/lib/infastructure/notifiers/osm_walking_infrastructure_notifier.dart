...
class OSMWalkingInfrastructureNotifier extends StateNotifier<NotifierState> {
  final OSMApi _osmApi;

  OSMWalkingInfrastructureNotifier(this._osmApi) : super(const Initial()) {
    getDetails();
  }
  ...
  getDetails() async {
    ...
    final response = await _osmApi.getWalkingInfastructure();
  ...
  List<Marker> _getRawMarkers(OSMCoordinatesDTO data) {
    ...
    for (var i = 0; i < data.coordinates.length; i += 1000) {
