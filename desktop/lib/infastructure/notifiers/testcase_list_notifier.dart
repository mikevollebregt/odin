...
import 'package:desktop/infastructure/notifiers/testcase_list_detail_notifier.dart';
...
class TestCaseListNotifier extends StateNotifier<NotifierState> {
  ...
  final TestCaseListDetailNotifier _testCaseListDetailNotifier;

  TestCaseListNotifier(this._testCaseApi, this._notifier, this._authApi, this._convertDataStructureService, this._testCaseListDetailNotifier)
      : super(const Initial()) {
    getListAsync();
  ...
  Future getListAsync() async {
    ...
    if (list.isNotEmpty) {
      _testCaseListDetailNotifier.getDetail(list[0].userId);
    }

    for (var userdata in list) {
      if (userdata.userTestDaysDataDTO == null) {
        continue;
      }
